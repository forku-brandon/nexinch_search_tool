...
$connect = new PDO("mysql:host=localhost;dbname=crudDB", "root", "");
...
if (isset($_POST["query"])) {
	...
	$query = "
	SELECT header FROM search-engine 
		WHERE header LIKE '%" . $condition . "%' 
		ORDER BY id DESC 
		LIMIT 10
	";
	...
	foreach ($result as $row) {
		$data[] = array(
			'header'		=>	str_ireplace($condition, $replace_string, $row["header"])
		);
	}
